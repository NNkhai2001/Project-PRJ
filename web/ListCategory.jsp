                            ...
                            <c:otherwise><!-- tuong duong else-->
                                <nav aria-label="Page navigation example mx-auto">
                                    <ul class="pagination justify-content-center">
                                        <li class="page-item ">
                                            <c:if test="${page != 1}">
                                                <a class="page-link" href="HomeController?do=ListCategory&page=${page-1}&cid=${cid}">Previous</a>
                                            </c:if>
                                        </li>
                                        <c:forEach begin="1" end="${totalPage}" var="i">                                                                   
                                            <li class="page-item ${i==page?"active":""}"><a class="page-link" href="HomeController?do=ListCategory&cid=${cid}&page=${i}">${i}</a></li>
                                            </c:forEach>  
                                        <li class="page-item">
                                            <c:if test="${page != totalPage}">
                                                <a class="page-link" href="HomeController?do=ListCategory&page=${page+1}&cid=${cid}">Next</a>
                                            </c:if>
                                        </li>
                                    </ul>
                                </nav>
                                    ...
                                    <div class="card h-100">
                                        ...
                                        <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                                            <c:if test="${L.unitsinstock > 0}">
                                                <div  class="text-center"><a class="btn btn-outline-dark mt-auto" href="AddToCartController?do=ListCategory&pid=${L.productid}&unitstock=${L.unitsinstock}&cid=${cid}&page=${page}">Add to cart</a></div>                                            
                                            </c:if >
                                            <c:if test="${L.unitsinstock == 0}">
                                                <div class="text-center"><a id="Het hang" onclick="myfuntion(this.id)" class="btn btn-outline-dark mt-auto" href="HomeController?do=ListCategory&cid=${cid}&page=${page}">Add to cart</a></div>                                            
                                            </c:if>
